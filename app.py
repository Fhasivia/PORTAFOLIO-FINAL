...
.pg{
  ...
  background:
    radial-gradient(ellipse 60% 45% at 0% 0%,rgba(0,255,140,.055) 0%,transparent 60%),
    radial-gradient(ellipse 40% 30% at 100% 100%,rgba(0,200,110,.04) 0%,transparent 55%),
    #050e09;
}
...
.nav{
  ...
  border-bottom:1px solid rgba(0,255,140,.08);
}
...
.nav-pill{
  ...
  border:1px solid rgba(0,255,140,.22);border-radius:100px;padding:5px 14px;
  font-family:'JetBrains Mono',monospace;font-size:.62rem;color:#00ff8c;letter-spacing:.1em;
  background:rgba(0,255,140,.06);
}
.nav-pill::before{
  content:'';width:6px;height:6px;border-radius:50%;
  background:#00ff8c;box-shadow:0 0 7px #00ff8c;
...
/* BASE CELL */
.c{
  border-radius:18px;border:1px solid rgba(0,255,140,.08);
  background:rgba(255,255,255,.022);padding:28px;
  position:relative;overflow:hidden;
  transition:border-color .25s,transform .22s,box-shadow .25s;
}
.c::before{
  content:'';position:absolute;inset:0;border-radius:18px;
  background:linear-gradient(135deg,rgba(0,255,140,.04) 0%,transparent 50%);
...
.hero-cta{
  ...
  padding:11px 24px;border-radius:100px;border:1px solid rgba(0,255,140,.3);
  background:rgba(0,255,140,.08);font-family:'JetBrains Mono',monospace;
  font-size:.7rem;color:#00ff8c;letter-spacing:.1em;text-decoration:none;
  transition:background .2s,border-color .2s;
}
.hero-cta:hover{background:rgba(0,255,140,.16);border-color:rgba(0,255,140,.55)}

/* STAT COLUMN */
.stat-col{grid-column:span 2;display:flex;flex-direction:column;gap:12px;background:transparent;border:none;padding:0}
.stat-col:hover{transform:none;box-shadow:none}
.stat-col::before{display:none}
.stat-box{
  flex:1;border-radius:16px;border:1px solid rgba(0,255,140,.08);
  background:rgba(255,255,255,.022);padding:18px 20px;
...
.about-c{grid-column:span 3} /* fills remaining col after hero(7)+stats(2) = needs 3 more => skills(3) but hero row = 7+2+3 = 12 ✓ */
/* second row: skills(3) about(3) fills under stats... actually we do new row */
...
/* SECTION ROW */
.sec-row{grid-column:span 12;background:transparent;border:none;padding:16px 0 4px;display:flex;align-items:center;gap:16px}
.sec-row:hover{transform:none;box-shadow:none;border-color:transparent}
.sec-row::before{display:none}
...
/* PROJECT CARDS */
.proj-c{grid-column:span 3;text-decoration:none!important;display:block;cursor:pointer}
...
for i, p in enumerate(projects):
    idx = str(i+1).zfill(2)
    tgs = "".join(f'<span class="proj-tag">{t}</span>' for t in p["tags"])
    proj_html += f"""
...
<div class="pg">
  ...
  <div class="grid">
    ...
    <div class="c hero-c">
      <div class="hero-eyebrow">// Data Science · ML · Computer Vision · 2025</div>
      ...
      <div class="hero-desc">
        Transformo datos en soluciones inteligentes. Especializada en visión por computadora,
        NLP, IoT y aplicaciones de IA desplegadas en producción.
      </div>
    ...
    <div class="c stat-col">
      <div class="stat-box">
        <div class="stat-n">16</div>
        <div class="stat-l">Apps en producción</div>
      </div>
      <div class="stat-box">
        <div class="stat-n">8+</div>
        <div class="stat-l">Tecnologías</div>
      </div>
      <div class="stat-box">
        <div class="stat-n">AI</div>
        <div class="stat-l">Especialidad core</div>
      </div>
    </div>
    ...
    <!-- ROW 1 continues — About fills the last 3 cols on same row as skills -->
    <div class="c about-c">
      <div class="c-lbl">// Sobre mí</div>
      <div class="c-ttl">Perfil</div>
      <div class="about-txt">
        Apasionada por convertir datos en experiencias útiles e impactantes.
        Trabajo con visión por computadora, NLP, IoT y despliegue de apps
    ...
    <div class="c quote-c">
      ...
      <div class="q-txt">
        Los datos son el nuevo lenguaje universal. Mi misión es traducirlos
        en herramientas que la gente pueda ver, sentir y usar en su día a día.
      </div>
  ...
  <div class="footer">
    <div class="footer-l">Laura Martínez Alzate · Portfolio 2025</div>
    <div class="footer-r">Built with Python &amp; Streamlit · Medellín 🇨🇴</div>
  </div>
